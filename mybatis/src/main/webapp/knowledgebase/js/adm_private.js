...
function get_all_private_file_table(data) {
	...
	for (var i = 0; i < all_pri_file.length; i++) {
		...
		var td_4 = "<td style='padding-top:15px;text-align:center'>"
				+ timeStampFormatDay(all_pri_file[i].fileUploadTime * 1000)
				+ "</td>";
		var td_5 = "<td style='text-align:center'><button class='button button-primary button-rounded button-small'"
				+ " bid='"
				+ all_pri_file[i].fileId
				+ "'  onclick='download(this)' path='"
				+ all_pri_file[i].fileUrl + "'>下载</button></td>";
		if (all_pri_file[i].fileIsVisible == "私有")
			...
			var td_6 = "<td style='padding-top:15px;text-align:center'>"
					+ all_pri_file[i].fileCategory + "</td>"
			// var td_5 = "<td><button class='btn btn-primary'
			// data-toggle='modal' data-target=''
			// onclick='delete_one_pick(this)'>删除</button></td>";
			// var td_6 = "<td ></td>";
			// var td_7 = "<td><button class='btn btn-primary'
			// data-toggle='modal' data-target=''
			// onclick='pre_file(this)'>预览</button></td>";
		var content = tr_begin + td_1 + td_2 + td_3 + td_4 + td_6 + td_5
				+ tr_end;
		$("#pri_file").append(content);
	}
	createNewPagination(data, "file_private",
			"/mybatis/FileInfoController/myuploadfile.do",
			get_all_private_file_table, "first_file_click", "last_file_click",
			"page-file-one", {})
}
/*
 * 条件查询文件
...
function fileinfo_search(obj) {
	...
	/*
	 * if(filetype==''){ get_all_private_file(); } else
	 * if((filetype!='')&&(filecate=='')&&(filechildcate=='')){
	 * $("#type").val(filetype); $("#cate").val(filecate); get_all_my_file(); }
	 * else if((filetype!='')&&(filecate!='')&&(filechildcate=='')){
	 * $("#type").val(filetype); $("#cate").val(filecate); get_all_my_file();
	 * }else if((filetype!='')&&(filecate!='')&&(filechildcate!='')){
	 * $("#type").val(filetype); $("#cate").val(filechildcate);
	 * get_all_my_file(); }
	 */
}
/*
...
function get_my_file(data) {
	...
	createNewPagination(data, "file_private",
			"/mybatis/FileInfoController/selectByCondition.do", get_my_file,
			"first_file_click", "last_file_click", "page-file-one", {})
}

function file_classify(obj) {
	$(obj).attr({
		"data-toggle" : "modal",
		"data-target" : "#changefileinfo"
	});
...
function delete_all_pick(obj) {
	...
	if (check_num_1 > 0) {
		...
		$("#delete_file").html("确认将选中文件放入垃圾箱么？删除后可在垃圾箱恢复！");
...
/*
 * //下载选中的 function down_all_pick(obj){ var check_num_1 = 0;
 * $("input[name=checkAll]").each(function(){ if($(this).is(':checked')){
 * check_num_1 += 1; } }); if(check_num_1 > 0){
 * $(obj).attr({"data-toggle":"modal","data-target":"#download"});
 * $("#check_file").html("确认下载 所选的文件么？"); $("#check_file").removeClass();
 * 
 * $("#down_click").removeAttr("onclick_name");
 * $("#down_click").attr("onclick_name","down_all"); }else{
...
/*
 * function down_all_selected(){ var down_num = 0;
 * $("input[name=checkAll]").each(function(){ if($(this).is(':checked')){
 * down_num += 1; } }); // console.log(check_num); var down_array = new
 * Array(down_num); $("input[name=checkAll]").each(function(index){
 * if($(this).is(':checked')){ down_array[index] =
 * $(this).parent().siblings()[0].value;
 * //console.log($(this).parent().next().innerHTML); } }); //
 * console.log(check_array); $.ajax({ type:'post',
 * url:"/mybatis/FileInfoController/down_check_file.do", dataType:"json",
 * traditional:true, data:{"check_file_arry":down_array},
 * success:function(data){ get_all_private_file(); } }) }
 * 
 * function down_one_pick(obj){ var down_one_file =
 * $(obj).parent().siblings()[1].innerHTML;
 * $(obj).attr({"data-toggle":"modal","data-target":"#download"});
 * $("#check_file").html("确认下载 "+down_one_file+" 么？");
 * $("#check_file").removeClass(); $("#check_file").addClass(down_one_file);
 * $("down_click").removeAttr("onclick_name");
 * $("#down_click").attr("onclick_name","down_one");
 *  }
 * 
 * //确认下载单个文件 function down_one_selected(){ var check_file =
 * $("#check_file").attr("class"); $.ajax({ type:'post',
 * url:"/mybatis/FileInfoController/down_check_file.do", dataType:"json",
 * data:{"check_file_array":check_file}, success:function(data){ if(data){
 * get_all_user_is_check(); } } }) }
 * 
 * function down_file_ok(){ var btn_info =
 * $("#down_click").attr("onclick_name"); if(btn_info == "down_one"){
 * down_one_selected(); }else if(btn_info == "down_all"){ down_all_selected(); } }
 */
/*
 * function down_file(obj){ var btn_name = $(obj); var file_name =
 * btn_name.parent().siblings()[1].innerHTML;
 * 
 * btn_name.attr("data-target","#download");
 * $("#download_file").html("是否下载："+file_name+"？");
 * $("#download_file").removeClass(); $("#download_file").addClass(file_name); }
 * 
 * function pre_file(obj){ var btn_name = $(obj); var file_name =
 * btn_name.parent().siblings()[1].innerHTML;
 * 
 * btn_name.attr("data-target","#preview");
 * $("#preview_file").html("是否预览："+file_name+"？");
 * $("#preview_file").removeClass(); $("#preview_file").addClass(file_name); }
 * //下载 function downloadFile(obj){ var attr_d =
 * $("#download_file").attr("class");
 *  } //预览 function previewFile(obj){ var
 * attr_p=$("preview_file").attr("class"); $.ajax() }
 */